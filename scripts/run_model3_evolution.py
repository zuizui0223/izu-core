"""Prospective design freeze and exclusive, source-checked model-3 execution.
...
SOURCES = ['scripts/model3_evolution.py','scripts/model3_reproduction.py',
           'scripts/run_model3_evolution.py',
           'docs/MODEL3_NATURAL_HISTORY_TIMESCALES_20260925.md',
           'docs/superpowers/plans/2026-09-25-model3-evolution.md']
...
def build_cases():
    cases = []
    environments = [('mainland',False,1.),('activity_only',False,.4),
                    ('community_only',True,1.),('both',True,.4)]
    arms = [('primary',256,48,400,['selected','neutral','fixed']),
            ('larger_population',64,192,400,['selected','neutral'])]
    for campaign,repeats,capacity,years,controls in arms:
        for environment,survival,selfing,start,control in itertools.product(
                environments,[0.,.75],[0.,.5],[.3,.7],controls):
            label,community,activity = environment
            for replicate in range(repeats):
                case = dict(campaign=campaign,environment=label,replicate=replicate,
                            seed=104729+7919*replicate,years=years,capacity=capacity,
                            island_community=community,activity=activity,
                            survival=survival,selfing=selfing,start=start,control=control)
...
def run(path, output):
    ...
    # artifacts, one file per experimental cell, rather than 28,672 tiny files.
    ...
    for group_index,(_,iterator) in enumerate(itertools.groupby(design['cases'],key=group_key)):
        ...
        results = []
        for case in cases:
            arguments = {k:v for k,v in case.items() if k not in ('case_id','campaign','environment','replicate')}
