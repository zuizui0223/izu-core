...
def render_submission_manuscript() -> str:
    ...
    for token in (
        ...
        "44–60/96",
        "0.134–0.172",
        "not an analytical mean-field or linear-noise limit",
    ):
...
def build_supporting_tables() -> str:
    ...
    for row in (TABLE_ROW_EQUAL_TURNOVER, TABLE_ROW_SYSTEM_SIZE):
        if row not in text:
