...
RESULT_SIZE_ADDITION = (
    ...
    "\n\nA complementary prespecified system-size audit retained the headline plant response operator (`trait_adjustment=0.03`) and reused the existing six-seed ensemble. The variance hierarchy itself reversed as community sampling noise was reduced: median starting-position share rose monotonically from 2.55% at k=1 through 10.33%, 27.33% and 42.52% to 55.84% at k=16, whereas median community-realization share fell from 72.98% at k=1 through 48.03%, 23.52% and 18.26% to 12.72% at k=16. Starting position exceeded community realization in 0/6 seeds at k=1 and k=2, but in 6/6 seeds at k=4, k=8 and k=16. Mixed-sign realizations nevertheless persisted at k=16 (28–42/96 across seeds). Thus the ordering of response determinants is itself regime dependent: realized community dominates in small stochastic communities, whereas starting state becomes the larger additive component as communities become larger and more stable under active plant adjustment."
)
