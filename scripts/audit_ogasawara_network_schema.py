...
def table_record(
    ...
    long_core = {"plant", "pollinator", "interaction_count"}.issubset(
        matched
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/ogasawara_zenodo/schema_audit.json"),
    )
    ...
    for path in paths:
        ...
        try:
            ...
            tables.append(
                {
                    "file": str(path),
                    "status": "table_audit_failed",
                    "error": repr(error),
                }
            )

    summary = {
        "status": "schema_candidates_found"
        if tables
        else "no_tabular_files_found",
        "n_tabular_units": len(tables),
        "n_contextual_long_candidates": sum(
            bool(row.get("contextual_long_network_candidate"))
            for row in tables
        ),
        "n_long_candidates": sum(
            bool(row.get("long_network_candidate")) for row in tables
        ),
        "n_wide_candidates": sum(
            bool(row.get("wide_interaction_matrix_candidate"))
            for row in tables
        ),
        "tables": tables,
        "next_gate": (
            "Select source-defined table(s), verify legitimate-interaction semantics "
            "and sampling effort, then implement a source-specific parser."
        ),
        "claim_boundary": (
            "Header matching is a schema-discovery aid, not biological classification. "
            "A visit or interaction count is not pollinator effectiveness, effective "
