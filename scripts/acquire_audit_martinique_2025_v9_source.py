...
USER_AGENT = "izu-core-martinique-v9-source-gate/1.0"
...
def role_candidates(headers: list[str]) -> dict[str, list[str]]:
    ...
    rules = {
        ...
        "effort": ("effort", "duration", "minute", "minutes", "hour", "transect", "census", "observation_time", "sampling_time"),
    }
    return {
...
def inspect_workbook(payload: bytes, name: str) -> dict:
    ...
    for sheet in book.worksheets:
        ...
        interaction_candidate = bool(
            roles["site"] and roles["time"] and roles["plant"]
            and roles["pollinator"] and roles["interaction_amount"]
        )
...
def main() -> None:
    ...
    for source in design["author_deposited_files"]:
        ...
        record = {"name": name, "url": source["url"], "http_status": status, "error": error}
    ...
    source_bytes_ok = len(blocked) == 0 and len(records) == len(design["author_deposited_files"])
    ...
    if not source_bytes_ok:
        status_name = "blocked_martinique_primary_bytes_not_recovered"
    ...
    output = {
        ...
        "claim_boundary": (
            "Source/schema admission only. Even a GO does not freeze monthly/bi-period reconstruction, identity handling, "
