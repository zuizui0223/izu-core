...
def render() -> str:
    ...
    body.append(text(40, 42, "Fig1. Island ecological response architecture", size=23, weight="bold"))
    ...
    body.append(text(180, 168, "simplification / reorganization", size=13, weight="bold", anchor="middle"))
    body.append(text(180, 196, "shared island-like perturbation", size=11, anchor="middle"))
    ...
    body.append(text(470, 198, "branch potential", size=11, anchor="middle"))
    body.append(line(305, 165, 345, 165))

    body.append(text(655, 90, "C  Local context and filters", size=16, weight="bold"))
    ...
    body.append(text(945, 90, "D  Ecological response states", size=16, weight="bold"))
    body.append(rect(925, 110, 245, 110))
    body.append(text(1047, 140, "branching", size=13, weight="bold", anchor="middle"))
    body.append(text(1047, 165, "same-direction propagation", size=12, weight="bold", anchor="middle"))
    body.append(text(1047, 190, "buffering / alternative", size=12, weight="bold", anchor="middle"))
    body.append(line(885, 165, 925, 165))

    body.append(text(70, 285, "Frozen mechanism tests", size=17, weight="bold"))

    body.append(rect(55, 310, 330, 120, fill="#fafafa"))
    body.append(text(220, 340, "Branch generator", size=15, weight="bold", anchor="middle"))
    body.append(text(220, 367, "initial functional-position heterogeneity OFF", size=12, anchor="middle"))
    body.append(text(220, 391, "mixed-sign runs: 0.4167 → 0", size=13, weight="bold", anchor="middle"))
    body.append(text(220, 414, "same boundary in independent seed block", size=11, anchor="middle"))

    body.append(rect(445, 310, 330, 120, fill="#fafafa"))
    body.append(text(610, 340, "Local interaction context", size=15, weight="bold", anchor="middle"))
    ...
    body.append(text(610, 414, "bidirectional branch allocation", size=11, anchor="middle"))
    ...
    body.append(text(1000, 414, "downstream attenuation, not sign reversal", size=11, anchor="middle"))

    body.append(text(70, 495, "Cross-island ecological challenge", size=17, weight="bold"))
    body.append(rect(55, 515, 1110, 85, fill="#f7f7f7"))
    body.append(text(610, 544, "13 strict systems: 3 branching / 6 propagation / 2 buffering-alternative / 1 axis decoupling / 1 retained failure", size=14, weight="bold", anchor="middle"))
    body.append(text(610, 572, "The external series tests recurrence of response architecture; it is not a prevalence sample and does not imply one shared empirical mechanism.", size=11, anchor="middle"))
    body.append(line(1047, 220, 1047, 515, dashed=True))

    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">\n'
        '<title>Island ecological response architecture</title>\n'
        + "\n".join(body)
        + "\n</svg>\n"
    )
