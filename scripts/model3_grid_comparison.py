"""Matched projected-founder individual and density trajectories.

Forward-ported from model3_robustness at16046e9; archived scripts are unchanged.
Only baseline reproductive settings are admitted for the density counterpart.
"""
...
from scripts.model3_meanfield import genotype_grid, project_founders, meanfield_step


def simulate_grid_pair(*, seed, points=3, years=80, activity=1., island_community=False,
    ...
    if (control not in ('selected','neutral') or pollen_scale!=1. or investment_cost!=.5
            or ovule_effort!='lifetime' or pollen_effort!='lifetime' or background_ratio!=1.):
        raise ValueError('density comparison requires baseline reproductive settings')
    grid,kernel=genotype_grid(start,points)
    ...
    genotype,initial_density=project_founders(genotype,start,points)
    density=np.zeros((years+1,len(grid)))
    density[0]=initial_density
    density_outcross=np.zeros(years)
    density_selfed=np.zeros(years)
    density_established=np.zeros(years)
    ...
    for year in range(years):
        ...
        deterministic=meanfield_step(density[year],grid,kernel,visitors,activity=activity,
                                    survival=survival,selfing=selfing,depression=depression,control=control)
        density[year+1]=deterministic['density']
        density_outcross[year]=deterministic['outcross']
        density_selfed[year]=deterministic['selfed']
        density_established[year]=deterministic['established']
        if len(genotype):
            traits = genotype.mean(axis=2)
            n = len(genotype)
            pairs = np.zeros((n,n))
    ...
    return dict(density=density,density_outcross=density_outcross,
                density_selfed=density_selfed,density_established=density_established,
                population=population,trait_mean=trait_mean,trait_variance=trait_variance,
