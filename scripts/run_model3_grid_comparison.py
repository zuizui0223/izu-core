"""Prospective matched individual/distribution freeze and source-checked execution.
...
from scripts.model3_grid_comparison import simulate_grid_pair as simulate

ROOT = Path(__file__).resolve().parents[1]
SOURCES = ['scripts/model3_evolution.py','scripts/model3_reproduction.py',
           'scripts/model3_meanfield.py','scripts/model3_grid_comparison.py',
           'scripts/run_model3_grid_comparison.py',
           'docs/MODEL3_GRID_COMPARISON_DESIGN_20260925.md']
...
def build_cases():
    cases=[]
    for points,repeats in [(3,64),(4,16)]:
        for environment,survival,start,control,capacity in itertools.product(
                [('mainland',False,1.),('both',True,.4)],[0.,.75],[.3,.7],
                ['selected','neutral'],[48,192]):
            label,community,activity=environment
            for replicate in range(repeats):
                case=dict(campaign='grid_comparison',variant=f'grid{points}',points=points,
                    environment=label,replicate=replicate,seed=104729+7919*replicate,
                    years=400,capacity=capacity,island_community=community,activity=activity,
                    survival=survival,start=start,control=control,selfing=.5,depression=.5)
...
def run(path, output):
    ...
    # artifacts, one file per experimental cell, rather than 2,560 tiny files.
