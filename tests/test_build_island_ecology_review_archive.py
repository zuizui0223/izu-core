...
def test_review_archive_file_list_excludes_identity_files_and_retired_drafts():
    assert MANUSCRIPT in CORE_REVIEW_FILES
...
def test_review_archive_builds_from_active_conditional_response_geometry_state(tmp_path: Path):
    ...
    with zipfile.ZipFile(output) as archive:
        ...
        assert "conditional response geometry" in manuscript.lower()
        ...
        assert manifest["manuscript_source"] == MANUSCRIPT
        assert manifest["scientific_state"] == "conditional_response_geometry_model_gate_closed"
        assert manifest["figures_regenerated_fail_closed"] is True
        assert "comparative grounding only" in manifest["claim_boundary"].lower()

        readme = archive.read("README_REVIEW_ARCHIVE.md").decode("utf-8").lower()
        assert "conditional response geometry" in readme
        assert "retired journal of ecology v2/v3 drafts" in readme
        for token in ["issue #91", "microdonta"]:
