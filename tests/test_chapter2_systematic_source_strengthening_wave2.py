...
def test_tasmania_is_cleared_without_creating_full_contract():
    payload = build_strengthening_audit()
    wave = payload["wave2"]
    assert wave["reviewed_targets"] == 2
    assert wave["cleared_from_source_work"] == 1
    assert wave["cleared_target_names"] == ["Tasmania"]
    assert wave["new_global_confrontation_candidates"] == 1
    assert wave["candidate_target_names"] == ["Tasmania"]
    assert wave["full_chapter2_contract_passes"] == 0
    assert payload["source_work_state_after_wave2"]["targets_requiring_additional_source_work"] == 47
