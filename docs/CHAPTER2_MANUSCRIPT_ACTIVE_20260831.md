...
**Status:** active Chapter 2 scientific manuscript — world-saturation / Izu-continuity synthesis; submission metadata still fail-closed
**Updated:** 2026-09-06
**Inference architecture:** model possibilities → world confrontation → identifiability bottleneck → focal Izu continuity-system zoom
**Controlling state:** `docs/CHAPTER2_CANONICAL_STORY_20260827.md`, `docs/CHAPTER2_RELATIONAL_ROBUSTNESS_CORRECTION_20260831.md`, `docs/CHAPTER2_WORLD_CONFRONTATION_SATURATION_AND_IZU_ZOOM_20260906.md`, `docs/CHAPTER2_IZU_FOCAL_SYSTEM_RATIONALE_20260906.md`, `data/design/chapter2_izu_focal_system_rationale_20260906.json`, `data/results/chapter2_scientific_gate_decision_frozen_20260827.json`, `data/results/chapter2_relational_robustness_audit_frozen_20260831.json`, `data/results/chapter2_izu_final_mechanistic_zoom_audit_20260906.json`
...
We exposed the interaction kernel in a frozen matching model, mapped island-minus-mainland service across matched starting positions, diagnosed regime transitions, decomposed starting-state and community contributions, and separated local filtering from downstream assurance. We then confronted this vocabulary with a source-audited 25-entry formal island literature inventory, tested whether the qualitative evidence vocabulary saturated under geography-first island-system expansion, and increased mechanistic resolution in Izu. Izu was selected only after that breadth audit, using cross-layer measurement continuity rather than geographic convenience, representativeness or agreement with the synthetic model.

Across 96 matched community realizations, 41 contained both positive and negative responses; mixed geometry also persisted when mainland-like and island-like initial pollinator richness was equalized (53/96). Partner loss and arrival were the strongest sign-stable regime associations. Across prespecified seed and time-horizon sensitivities, realized community remained the largest additive component, while starting state alone remained weak and state-by-community non-additivity consequential; mixed geometry persisted even with trait adjustment set to zero. Local filtering reallocated branches asymmetrically, whereas assurance produced no sign rescue among 580 eligible declines. The frozen 25-entry audit remained outcome-rich but process-poor: responses were directly measured in 21/25 entries, partner arrival/replacement in only 2/25, and no entry supplied the full outcome-independent contract. A geography-first >=20 km2 expansion then reached a preregistered two-tranche zero-novelty stopping rule; an independent small-island supplement recovered a direct partner-reintroduction coordinate on Tiritiri Matangi and a dated founding chronology on Surtsey, but still no full matched transition contract. In Izu, contemporary pollinator functional diversity predicted corrected trait matching in the five-island set and within the four post-Oshima islands, with all leave-one-island coefficients positive. Matching-to-pollen associations remained positive on average but were not leave-one-island sign stable, and eight shared plant targets with lower corrected matching branched into three shorter, four longer and one unchanged floral-tube responses and four lower versus four higher pollen-receipt responses.

Response direction is therefore relational rather than intrinsic: organismal state must be evaluated against the community realized after reorganization. Geography-first expansion shows that the empirical measurement bottleneck is structured rather than a consequence of one literature frame; small islands partially recover transition history, while the Izu zoom resolves the contemporary functional-realization half of the chain. Izu is valuable precisely because the same regional system retains historical response evidence, repeated contemporary networks, quantitative functional traits, explicit competing historical explanations and model-facing failures as well as successes. Historical partner turnover linked to a matched pre-transition state and post-transition plant response remains the unresolved coordinate and is handed forward rather than retrofitted into Chapter 2.
...
We then confront the response vocabulary with a source-audited comparative universe. Its purpose is first to ask whether real island responses require branching, propagation, buffering, axis decoupling and retained falsification as distinct states. Its second purpose is to identify which source-native measurements are missing when those mechanisms cannot be distinguished. We do not project systems into synthetic regimes from known outcomes, treat research entries as exchangeable archipelagos or reconstruct stopped predictors post hoc. After the formal 25-entry audit was frozen, a separate geography-first expansion was used only to test whether continued coverage added new response, process or falsification states and whether the same measurement bottleneck persisted outside the original literature-built frame.

The move from breadth to Izu is therefore an inferential decision, not a geographic convenience. Once geography-first expansion stopped adding materially new mechanism states, the appropriate focal system was the one that could connect the largest portion of the unresolved chain within one source-linked regional series. Izu provides historical *Campanula* reproductive and floral-response evidence, repeated contemporary plant–pollinator networks, numeric pollinator functional traits and downstream reproductive-function measurements, while retaining explicit population-history alternatives and important negative model-facing results. That combination permits competing explanations to be separated rather than merely illustrated. The directly measured *Campanula microdonta* phenotype remains the independent empirical endpoint of Chapter 3 and is not imported here as validation or as a reason to tune the Chapter 2 mechanism.
...
2. **World confrontation and identifiability audit.** External island research entries test whether a one-direction vocabulary is empirically adequate and whether the joint measurements needed for formal comparison are available. They are not treated as a meta-analysis, prevalence sample or validation coverage. Geography-first expansion after the formal audit is a saturation/measurement audit, not a new prediction denominator.
3. **Focal Izu continuity-system zoom.** Source-locked secondary analyses use published Izu plant–pollinator data to separate historical signed-position inference, contemporaneous functional structure, trait matching and downstream pollen response. Izu does not calibrate synthetic thresholds and is not an outcome-independent global ranking winner.
...
To test whether that measurement asymmetry was an artefact of the literature-built search frame, we then opened a separate geography-first coverage audit without changing the frozen 25-entry formal denominator. A historical Global Islands v3 / WCMC-USGS-derived artifact supplied 4,663 candidate island polygons >=20 km2. Raw polygons were grouped into archipelago or biogeographically coherent system units before literature review; nested islands were not counted as independent systems merely because multiple polygons existed. Later tranches contained at least eight new systems selected before their literature outcomes were known. Before those searches, we froze a stopping rule: two consecutive eligible tranches with no new response state, process state, falsification role, direct historical partner loss/arrival measurement or full contract would constitute large-island saturation for the declared coverage procedure. A separate geography-first <20 km2 supplement was required before the final Izu zoom. Search rows remained descriptive/saturation evidence and were not silently promoted into the formal external-prediction denominator.
...
The required continuity was: historical focal-lineage response evidence; repeated contemporary interaction structure; quantitative pollinator functional traits; present functional matching and reproductive-function information; an independently measurable downstream phenotype; and a feasible route to visitor effectiveness, reproductive dependency and mature reproductive output in the same tagged populations. Negative or fragile model-facing results were not grounds for exclusion. The full rationale was frozen in `data/design/chapter2_izu_focal_system_rationale_20260906.json` before this manuscript closure step.

Izu meets that criterion because the same regional series contains historical *Campanula microdonta* floral, mating-system and autonomous-capacity responses; three mainland, one Oshima bridge and four post-Oshima sites in a repeated contemporary network programme; source-native numeric pollinator traits for most named taxa; present matching and pollen-response measurements; explicit colonisation/history alternatives; and a separate five-island focal phenotype programme reserved for Chapter 3. The last layer is not used to validate or tune Chapter 2; its role is to ensure that the downstream phenotype is independently measurable in the same lineage.
...
We separately used the source-native 2024 site × season network metrics to test contemporary functional realization directly rather than infer it from the historical signed-position projection. Corrected trait matching (`TM_z`) was related to pollinator functional diversity (`FDQ`) with site and season fixed effects, first across the five Izu islands and then within the four post-Oshima islands; leave-one-island fits tested sign dependence on any single site. Pollen receipt was aggregated to plant × site × season cells and evaluated against `TM_z` in corresponding fixed-effect sensitivities. Finally, eight plant targets shared across the source-defined Oshima-to-post contrast were compared across corrected matching, floral-tube response and pollen receipt. These analyses diagnose contemporary association and response propagation; they do not identify historical FDQ change, *Bombus* loss or a causal second-boundary effect.
...
The literature-built 111-target search frame was not treated as a census of world islands. Geography-first screening against the independent >=20 km2 island master readily recovered further contemporary process and outcome information but continued to miss the historical transition coordinates. In the first 20 omitted systems, direct plant response was available in 10/20, breeding/assurance in 9/20, realized-community or process contrast in 7/20 and local filtering in 2/20, whereas direct historical partner loss and partner arrival/replacement were both 0/20 and full contracts remained 0/20.

The preregistered stopping rule was not met immediately. A third geography-first tranche added a new historical/ploidy alternative mechanism in the Gulf of California and reset the saturation count. The next two eligible tranches, each selected before literature outcomes were known, added no new response, process or falsification state and no full contract; the >=20 km2 expansion therefore reached the declared zero-novelty 2/2 stopping condition.

The required <20 km2 supplement moved the bottleneck one step without closing it. Among eight preselected small-island systems, Tiritiri Matangi supplied a direct dated partner-reintroduction history and Surtsey supplied a partially bounded empty-island colonization chronology; direct historical partner loss remained 0/8 and full contracts remained 0/8. Thus transition history can be recovered in special small-island settings, but a matched source state → transition → realized community → plant-response chain still did not close. These geography-first searches did not reopen the frozen 25-entry formal denominator or the `not_evaluable` external-prediction decision.

## Resolution: Izu localized the raw signed-position signal to source state and composition

The frozen Izu signed-position projection was positively associated with realized raw trait matching across 83 plant × island-site rows and 30 plant clusters: slope=0.5669, cluster-robust SE=0.1316 and 95% CI=0.2977–0.8361 (Fig. 4B). Correct plant source-state identity mattered: none of 10,000 fixed-seed source-position permutations reached the observed raw slope.
...
Using the same frozen signed-position predictor and rows, the source paper's background-community-corrected matching response was unsupported: slope=0.0333, cluster-robust SE=0.1473, 95% CI=−0.2680–0.3346, and 3918/10,000 null source-position permutations were at least as large as the observed corrected slope.

This negative result concerns the historical signed-position projection as a predictor of beyond-background sorting. It does not imply that contemporaneous pollinator functional structure is unrelated to the source paper's corrected trait-matching metric. Increasing resolution therefore separates historical-transition inference from present community structure rather than treating either as validation of the synthetic transition surface.

## Resolution: contemporary functional diversity predicted corrected matching within Izu

In the source-native 2024 site × season networks, pollinator functional diversity (`FDQ`) was positively associated with corrected trait matching (`TM_z`) after site and season fixed effects. The FDQ coefficient was +1.943 across the five Izu islands and +2.059 within Niijima, Kozu, Miyake and Hachijo alone. All five leave-one-island coefficients in the Izu set and all four leave-one-island coefficients in the post-Oshima subset remained positive.

The archived network table contained no recorded *Bombus* species × site × season rows in the four-island post-Oshima subset, although this is a sampled-network statement rather than proof of biological absence. The within-subset FDQ result therefore shows that continuous contemporary pollinator functional structure carries explanatory information that cannot be reduced to a binary sampled-*Bombus* label. It remains observational and does not identify historical *Bombus* loss as the cause.

## Resolution: matching-to-pollen propagation was weaker and downstream responses branched

Corrected trait matching was positively associated with pollen receipt on average in the same source system, with fixed-effect coefficients of +0.0353 across the five Izu islands and +0.0342 within the four post-Oshima islands. Unlike the FDQ-to-matching link, however, leave-one-island coefficient ranges crossed zero in both subsets. The downstream matching-to-pollen relation is therefore directionally consistent on average but materially more network-state dependent.

The plant-level cross-channel comparison made that attenuation concrete. All eight source-defined Oshima-to-post targets had lower corrected matching post-Oshima, yet floral-tube responses split into three shorter, four longer and one unchanged, and pollen receipt split into four lower and four higher. Only two of eight targets followed the complete matching-lower + tube-shorter + pollen-lower combination. A shared contemporary functional shift therefore does not imply a universal downstream floral or reproductive syndrome.
...
The stronger contribution of the source audit is its measurement asymmetry. Outcomes are commonly recorded, whereas the causal-side coordinates needed by the response geometry are much rarer. Most strikingly, partner arrival/replacement is directly measured in only 2/25 frozen formal entries even though arrival opposes loss in the strongest synthetic regime diagnostic. Source state and loss are also sparse. The formal literature inventory is therefore outcome-rich but process-poor for this particular mechanistic question.

The geography-first expansion shows that this asymmetry is not simply an artefact of having started from those 25 papers or the later 111 named targets. New large-island systems repeatedly added current visitor/community states, breeding-system measurements and plant outcomes while failing to add the historical turnover coordinates. Under an outcome-independent stopping rule, two consecutive geography-first tranches eventually added no new response/process/falsification state, so further >=20 km2 expansion ceased to change the mechanism vocabulary. The small-island supplement was more informative: Surtsey added a dated founding chronology and Tiritiri Matangi added a directly documented pollinator reintroduction plus a functional pollination test. Yet even these unusually history-rich systems lacked the matched pre-transition functional baseline needed to close the full longitudinal contract.

This makes the zero full-contract count interpretable rather than merely disappointing. Existing studies often document that a response occurred but do not jointly observe source state, loss, arrival/replacement, community functional change, local filtering and a comparable response on the same transition unit. Formal external prediction remains `not_evaluable`; the expanded audit instead localizes the missing information to longitudinal transition measurement. The final synthesis therefore retains only distinct roles—dated founding state, documented reintroduction with buffering, and a historical/ploidy alternative mechanism—rather than inflating the paper with every searched island.

## Why Izu is the focal continuity system

After world breadth saturates, the next system should not be chosen because it is nearby, typical or visually compelling. It should be chosen because it can resolve the specific measurements that broad comparison leaves disconnected. Izu meets that criterion unusually well within this programme: one regional island series links historical *C. microdonta* reproductive and floral responses, repeated contemporary interaction networks, source-native pollinator functional traits, present trait matching and pollen receipt, and an independently measurable five-island phenotype reserved for Chapter 3.

The historical layers are also deliberately non-clean. Ordered flower-size and outcrossing erosion remain compatible with colonisation and demographic history, whereas autonomous reproductive capacity changes more sharply across the northern island boundary. The frozen signed-position analysis organizes raw matching but fails for null-corrected matching, and the Oshima-source sensitivity is unsupported. These are strengths of the focal design rather than reasons to abandon it: they prevent Izu from functioning as a hand-picked positive control and force alternative explanations to remain visible.

The contemporary network analysis resolves a different layer. FDQ predicts corrected trait matching within Izu and even within the four post-Oshima islands, with leave-one-island sign stability. Thus realized functional community structure has a robust contemporary association with matching even where the sampled network table contains no *Bombus* rows. Propagation then weakens: matching-to-pollen is positive on average but not leave-one-island sign stable, and the eight shared plant targets branch strongly in floral-tube and pollen response despite a common decline in corrected matching.

This is mechanistic resolution rather than failed validation. World confrontation exposes the historical-transition bottleneck; Izu resolves the contemporary functional-realization and response-branching half of the chain. The unresolved piece is now precise: a direct historical partner transition linked to the same population's pre-transition state, realized visitor/effectiveness structure and post-transition reproductive response.

## Chapter 2 closes at the continuity-system boundary

At the dissertation scale, Chapter 1 asks when and where isolation-associated response vectors differ. Chapter 2 asks how a broad interaction reorganization can generate different outcomes, demonstrates that the response vocabulary and measurement bottleneck persist through geography-first world expansion, and then identifies Izu as the source-linked system in which the contemporary half of that mechanism can be resolved without erasing contrary evidence.

Chapter 3 owns the direct focal phenotype. The `shimahotarubukuro` analysis measures *C. microdonta* across five Izu islands and asks how much divergence is a coordinated size/investment trajectory versus residual departure from common allometry. Those phenotype results are not imported into Chapter 2 as model validation or historical-cause evidence. Their role in the dissertation architecture is to provide the downstream realized phenotype that Chapter 2 says cannot be predicted from a one-direction interaction syndrome.

A still stronger causal bridge would link, in the same tagged populations, usable observation effort including zero-visit windows to visitor identity and site-linked proboscis traits, legitimate floral contact, single-visit pollen deposition with no-visit controls, open/bagged-autonomous/supplemental-outcross reproduction and mature fruit or seed. Historical identification additionally requires either a true temporal partner-regime transition with matched pre/post plant response or an independently replicated bridge-state exposure. More species or seasons at the single Oshima bridge site can improve precision but cannot create an independent geographic boundary replicate. These are the next empirical measurements, not prerequisites for the Chapter 2 mechanism/identifiability conclusion.

## Limits

The synthetic coordinate and sign transitions are not empirical trait thresholds. The joint-design coefficients are not field causal effects. The audited horizon and seed ensembles are sensitivity sets, not natural time scales or population sampling distributions. Equalizing initial pollinator richness leaves other mainland-like/island-like scenario differences intact. The frozen external-system registry is not a prevalence sample or held-out prediction set, and the geography-first expansion is a saturation audit rather than a world census. Izu is not claimed to be representative of all islands or selected by a formal global winner score. The Izu secondary analyses remain observational: species-level proboscis values are recovered for 202/209 current named taxa, but exact site-specific traits are incomplete; the Oshima bridge has one independent geographic unit; the matching-to-pollen analysis is not mediation; and no tagged per-visit effectiveness-to-mature-seed chain or temporal pre/post partner transition is currently available.

# Conclusion

Community reorganization can define a response geometry without determining one biological outcome. Turnover deforms that geometry; response direction emerges from organismal state evaluated against the realized community; local filtering reallocates branches; and assurance modifies downstream magnitude. This relational architecture survives changes in seed ensemble and model horizon, persists without trait adjustment, and does not require reduced initial pollinator richness. Geography-first world confrontation reaches an outcome-independent saturation point while preserving the same structured measurement gap: contemporary community states and plant outcomes are comparatively accessible, whereas historical partner loss/arrival linked to matched pre/post response is rare. Small islands show that transition history can be recovered but do not yet close the full contract.

Izu is the appropriate final depth axis not because it is local, convenient or representative, but because the same source-linked island series can place historical reproductive response, repeated contemporary communities, quantitative functional traits, present matching and pollen response, explicit historical alternatives and an independently measured focal phenotype into one continuity framework. It is retained even where model-facing predictions weaken, so the zoom functions as a mechanism discriminator rather than a positive example. Within Izu, contemporaneous functional diversity robustly organizes corrected matching, whereas propagation to pollen is weaker and plant responses branch; the older signed-position projection remains descriptive rather than historical-causal.

Chapter 2 therefore closes with a conditional response geometry, a world-level saturation/identifiability result and a justified continuity system for depth. The direct *C. microdonta* phenotypic realization belongs to Chapter 3, along with any new effectiveness/dependency measurements needed to close the remaining transition-linked bridge. The Chapter 2 contribution is a mechanistic coordinate system plus a sharply localized measurement contract—not a calibrated island predictor, natural-frequency estimate or ultimate historical explanation.

## Main figure captions

**Figure 1. Breadth-to-depth mechanistic-resolution funnel.** The synthetic model defines ecological possibilities and a trajectory-conditioned interaction-kernel coordinate; the frozen comparative audit establishes response diversity and an outcome-rich/process-poor identifiability bottleneck; geography-first expansion reaches a declared saturation rule while small islands partially recover transition history; Izu is selected after saturation by measurement continuity rather than convenience or positive model fit; the Izu zoom separates historical signed-position inference from robust contemporary FDQ-to-matching structure and weaker downstream propagation; the directly measured focal phenotype and remaining transition-linked effectiveness-to-reproduction bridge belong to Chapter 3.
...
**Figure 4. From outcome-rich literature to Izu mechanistic resolution.** (A) Direct-measurement availability across 25 source-audited research entries: response outcomes are common whereas source state, loss and especially arrival/replacement are sparse; zero entries meet the full joint contract. Counts are research-entry availability, not independent-archipelago frequencies, and geography-first expansion does not alter this frozen denominator. (B) The same frozen Izu signed-position projection is associated with raw realized matching but not null-corrected matching; exact island-centre magnitudes remain non-unique and the prespecified Oshima bridge is unsupported. Contemporary FDQ-to-`TM_z` and matching-to-pollen sensitivities are separate source-native diagnostics of present functional realization and downstream propagation.

## References

Use the source-audited active reference ledger in `docs/ISLAND_ECOLOGY_RESEARCH_ARTICLE_REFERENCE_LEDGER_20260827.md`. Hiraiwa & Ushimaru (2017, 2024) are the empirical sources for the Izu triangulation. Post-saturation world-system sources and their admission roles are locked in `data/design/chapter2_global_master_manuscript_value_review_20260906.csv` and `docs/CHAPTER2_WORLD_CONFRONTATION_SATURATION_AND_IZU_ZOOM_20260906.md`; they remain synthesis/supporting evidence rather than validation coverage. The Chapter 3 focal phenotype is maintained separately in `zuizui0223/shimahotarubukuro` and is not part of the Chapter 2 validation surface.
