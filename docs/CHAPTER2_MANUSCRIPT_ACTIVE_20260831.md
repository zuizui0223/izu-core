...
**Status:** active Chapter 2 scientific manuscript — world-saturation / final-Izu synthesis; submission metadata still fail-closed
**Updated:** 2026-09-06
...
**Controlling state:** `docs/CHAPTER2_CANONICAL_STORY_20260827.md`, `docs/CHAPTER2_RELATIONAL_ROBUSTNESS_CORRECTION_20260831.md`, `docs/CHAPTER2_WORLD_CONFRONTATION_SATURATION_AND_IZU_ZOOM_20260906.md`, `data/results/chapter2_scientific_gate_decision_frozen_20260827.json`, `data/results/chapter2_relational_robustness_audit_frozen_20260831.json`, `data/results/chapter2_izu_final_mechanistic_zoom_audit_20260906.json`
...
Response direction is therefore relational rather than intrinsic: organismal state must be evaluated against the community realized after reorganization. Geography-first expansion shows that the empirical measurement bottleneck is structured rather than a consequence of one literature frame; small islands partially recover transition history, while the Izu zoom resolves the contemporary functional-realization half of the chain. Historical partner turnover linked to a matched pre-transition state and post-transition plant response remains the unresolved coordinate.
...
That audit motivates, rather than competes with, the Izu focal analysis. Izu was not selected as an outcome-independent winner from a global ranking. It is the island series in which source floral state, source and island pollinator composition, numeric pollinator traits, interaction structure, trait matching and reproductive-function measurements can be connected at higher resolution. The contrast among a frozen historical signed-position projection, contemporaneous functional-diversity structure and downstream pollen receipt attacks a specific mechanistic distinction: historical-transition inference versus present community-composition structure versus propagation to plant response. Chapter 2 ends at this distinction and at the measurements still required to link historical partner turnover to tagged contemporary dependency and mature-seed outcomes; Chapter 3 is the next measurement stage, not causal validation of the present model.
...
3. **Focal Izu mechanistic-resolution zoom.** Source-locked secondary analyses use published Izu plant–pollinator data to separate historical signed-position inference, contemporaneous functional structure, trait matching and downstream pollen response. Izu does not calibrate synthetic thresholds and is not an outcome-independent global ranking winner.
...
## Izu increases resolution and separates historical inference from present mechanism

Izu supplies the deepest current bridge because the same archipelago contains published floral traits, pollinator functional traits, contemporary interaction structure and plant reproductive-function measurements. This is a transparent data-depth rationale, not an outcome-independent ranking.

The frozen signed-position analysis remains useful but bounded. Correct source identity organizes raw matching change, yet exact island-centre magnitudes are non-unique, a source-position-only model is at least as descriptive as the full centre-shift projection, the prespecified Oshima-source bridge is unsupported and the signed-position association disappears for background-community-corrected matching. The historical projection therefore does not identify *Bombus* loss or a uniquely ordered island transition.
...
## Chapter 2 hands a transition-linked measurement contract to Chapter 3

At the dissertation scale, Chapter 1 asks when and where isolation-associated response vectors differ. Chapter 2 asks how a broad interaction reorganization can generate different outcomes, confronts that response geometry with empirical diversity, shows where world evidence saturates, identifies why existing comparisons cannot distinguish historical transitions and then resolves contemporary functional structure and propagation in Izu.

The remaining contract is prospective and plant linked. In the same tagged populations, usable observation effort including zero-visit windows should be linked to visitor identity and site-linked proboscis traits, legitimate floral contact, single-visit pollen deposition with no-visit controls, open/bagged-autonomous/supplemental-outcross reproduction and mature fruit or seed. Historical identification additionally requires either a true temporal partner-regime transition with matched pre/post plant response or an independently replicated bridge-state exposure. More species or seasons at the single Oshima bridge site can improve precision but cannot create an independent geographic boundary replicate. Chapter 3 advances to this higher-resolution focal measurement; its future observations are not retroactively treated here as model validation, *Bombus*-causation proof or external-prediction success.

## Limits

The synthetic coordinate and sign transitions are not empirical trait thresholds. The joint-design coefficients are not field causal effects. The audited horizon and seed ensembles are sensitivity sets, not natural time scales or population sampling distributions. Equalizing initial pollinator richness leaves other mainland-like/island-like scenario differences intact. The frozen external-system registry is not a prevalence sample or held-out prediction set, and the geography-first expansion is a saturation audit rather than a world census. The Izu secondary analyses remain observational: species-level proboscis values are recovered for 202/209 current named taxa, but exact site-specific traits are incomplete; the Oshima bridge has one independent geographic unit; the matching-to-pollen analysis is not mediation; and no tagged per-visit effectiveness-to-mature-seed chain or temporal pre/post partner transition is currently available.

# Conclusion

Community reorganization can define a response geometry without determining one biological outcome. Turnover deforms that geometry; response direction emerges from organismal state evaluated against the realized community; local filtering reallocates branches; and assurance modifies downstream magnitude. This relational architecture survives changes in seed ensemble and model horizon, persists without trait adjustment, and does not require reduced initial pollinator richness. Geography-first world confrontation reaches an outcome-independent saturation point while preserving the same structured measurement gap: contemporary community states and plant outcomes are comparatively accessible, whereas historical partner loss/arrival linked to matched pre/post response is rare. Small islands show that transition history can be recovered but do not yet close the full contract. In Izu, contemporaneous functional diversity robustly organizes corrected matching, whereas propagation to pollen is weaker and plant responses branch; the older signed-position projection remains descriptive rather than historical-causal. The contribution is therefore a mechanistic coordinate system plus a sharply localized measurement contract for conditional post-establishment response, not a calibrated island predictor, natural-frequency estimate or ultimate historical explanation.

## Main figure captions

**Figure 1. Breadth-to-depth mechanistic-resolution funnel.** The synthetic model defines ecological possibilities and a trajectory-conditioned interaction-kernel coordinate; the frozen comparative audit establishes response diversity and an outcome-rich/process-poor identifiability bottleneck; geography-first expansion reaches a declared saturation rule while small islands partially recover transition history; the Izu zoom separates historical signed-position inference from robust contemporary FDQ-to-matching structure and weaker downstream propagation; the remaining transition-linked effectiveness-to-reproduction contract is handed to Chapter 3 without validation claims.
