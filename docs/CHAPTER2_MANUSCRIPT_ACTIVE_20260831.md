...
Using a frozen plant–pollinator matching model, we separated realized-richness effects from compositional effects and partitioned response variation into starting-state, community-realization and state × community components. We then pooled independent community trajectories across `k={1,2,4,8,16}` while retaining the same response operator. A source-audited metadata and secondary-data layer was used as an external confrontation of biological plausibility and identifiability; no natural system was used to calibrate the synthetic coordinate or crossover.
...
The metadata confrontation exposed the natural claim ceiling rather than supplying a retrospective calibration: 21/25 formally audited research entries measured comparable plant responses, only 2/25 directly measured partner arrival or replacement, and 0/25 closed the full source-state → transition → realized-community → response contract. Existing Izu secondary analyses likewise contained both support and failure: functional exposure predicted corrected matching robustly, whereas translation from matching to pollen was not leave-one-island sign stable and historical signed-position projection was unsupported after null correction.

Together, these results show that community reorganization has no fixed determinant hierarchy. Realized richness helps position the ensemble regime, realized composition controls much of the variation in small stochastic communities, and starting state becomes increasingly important as community sampling stabilizes. Branching disappears only in the deterministic mean-field limit. Island syndromes can therefore be interpreted as ensemble-level regime shifts rather than deterministic lineage-level trait rules, while source-audited natural evidence constrains how far that mechanism can currently be transported.
