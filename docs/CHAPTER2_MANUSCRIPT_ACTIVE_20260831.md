...
The formal source-audited identifiability universe contains 13 strict external state challenges and 12 additional analytical or model-development entries; these 25 research entries are not independent archipelagos. We retained the qualitative state vocabulary already present in the registry—same-direction propagation, branching, buffering, axis decoupling and retained falsification—as a reality-necessity test. Known outcomes were not used to infer missing predictors or assign systems to synthetic regimes.

That formal denominator was frozen before later search expansion. A subsequent systematic review covered 111 named island targets and, after source strengthening and manuscript-value screening, expanded the descriptive universe to 42 research entries across 37 exact geographic labels. These later entries were not added to the frozen 25-entry measurement fractions, were not treated as independent archipelagos and did not reopen formal external prediction.
...
After the frozen audit, the systematic programme broadened descriptive coverage to 42 research entries across 37 exact geographic labels. Manuscript-value screening retained three non-redundant cases for direct discussion. On Crete, *Cyclamen creticum* is self-compatible but cannot set seed without pollinators, separating compatibility from autonomous assurance (Affre & Thompson, 1997). Trinidad and Tobago provide a 13-month inter-island hummingbird-diversity contrast linked to differences in visitation, pollen loads and nectar traits (Feinsinger et al., 1982), supplying a direct community-context contrast without implying the synthetic mechanism. In Iceland, *Campanula uniflora* shows pre-anthesis cleistogamy and predominant inbreeding under severe Arctic pollinator constraints (Ægisdóttir & Thórhallsdóttir, 2006), demonstrating a distinct assurance route while keeping abiotic context as a competing explanation. These cases strengthen falsification and mechanism breadth; they do not change the frozen 25-entry identifiability result.
...
The post-freeze expansion sharpens why breadth is not equivalent to validation. Crete blocks a shortcut from self-compatibility to autonomous assurance; Trinidad and Tobago show that realized pollinator-diversity differences can map to plant pollination biology across islands; Iceland shows a different assurance solution under strong abiotic constraint. Together these cases increase contrast among plausible pathways rather than the number of independent tests of one pathway. The descriptive universe is now 42 research entries across 37 exact geographic labels, while formal identifiability remains frozen at 25 entries and formal external prediction remains `not_evaluable`.
...
Use the source-audited active reference ledger in `docs/ISLAND_ECOLOGY_RESEARCH_ARTICLE_REFERENCE_LEDGER_20260827.md`. Hiraiwa & Ushimaru (2017, 2024) are the empirical sources for the Izu triangulation. Affre & Thompson (1997), Feinsinger et al. (1982) and Ægisdóttir & Thórhallsdóttir (2006) are cited only for the three value-selected breadth roles above. Other external-system references remain in the comparative-grounding supplement and are not presented as validation coverage.