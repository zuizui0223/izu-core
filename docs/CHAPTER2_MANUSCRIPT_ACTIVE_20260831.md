...
**Status:** active Chapter 2 scientific manuscript — simulation + source-audited metadata/secondary-data confrontation complete; field E3/E4 reserved for post-Chapter-2 transport
**Updated:** 2026-09-12
**Inference architecture:** conditional response geometry → realized-richness control → system-size determinant-rank crossover → downstream modifiers → metadata confrontation / bounded empirical claim ceiling
...
Ecological responses to community reorganization are often summarized by directional means and fixed driver hierarchies, yet both can fail when realized communities are finite and stochastic. We asked how the same broad pollinator-community reorganization can generate opposing plant-response branches and whether the dominant source of variation changes as community sampling stabilizes.

Using a frozen plant–pollinator matching model, we separated realized-richness from compositional effects, partitioned response variation into starting-state, community-realization and state × community components, and pooled independent community trajectories across `k={1,2,4,8,16}`. Exact stepwise richness matching made the ensemble mean all-positive in all six prespecified seeds, yet 51–65/96 realizations remained mixed and non-additivity remained 42.72–48.51%. Under active adjustment, median starting-state share rose from 2.55% at `k=1` to 55.84% at `k=16`, whereas community-realization share fell from 72.98% to 12.72%; mixed branching persisted at `k=16`.
...
Community reorganization therefore has no fixed determinant hierarchy. Richness can position the ensemble regime without determining individual branch identity, and the dominant source of variation can shift from community realization toward starting state as stochastic community variation contracts. Source-audited natural evidence makes these ingredients biologically non-vacuous while defining the empirical claim ceiling.
...
The source-audited natural evidence is informative precisely because it contains both support and failure. The formal audit shows that response outcomes are common while directly observed transition processes are rare, and the broader search reached its stopping rule without closing the missing longitudinal contract. Existing Izu secondary analyses then show that a relationship can be robust at one mechanistic step—functional exposure to corrected matching—while becoming unstable or branching at downstream steps. This is the empirical pattern expected if interaction reorganization cannot be compressed into one universal response axis, but it does not estimate the synthetic interaction term or determinant crossover in nature.
...
The result is a response geometry, not a calibrated field threshold. Local filtering and assurance occupy downstream positions in that architecture. Source-audited world evidence and existing Izu secondary data make the modeled ingredients biologically non-vacuous while retaining explicit adverse results and the `0/25` full-contract boundary; they therefore close the empirical claim ceiling without being misrepresented as full validation. Same-block Izu E3/E4 measurements remain an optional future validation programme only outside the present chapter—as a post-Chapter-2 transport/falsification study, not a prerequisite for current-paper closure.
