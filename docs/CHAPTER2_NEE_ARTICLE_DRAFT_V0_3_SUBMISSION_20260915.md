...
Island interaction systems provide a stringent external setting because insularity can reorganize partner breadth, composition, turnover and stochasticity while comparative studies often seek general directional or determinant-level conclusions. We do not test a universal island-syndrome effect. Instead, we use island systems to ask whether breadth and synchrony form a real two-dimensional context space and whether the existing literature measures enough of the determinant–outcome chain to transport rankings across that space.
...
Variance equivalence fixes `kappa_2`, but not `kappa_3`, `kappa_4` or higher cumulants. Under a common-factor construction, equal `k_eff` therefore leaves higher cumulants free even when pooled variance is identical.
...
The final primary plane contains **42 systems from six studies and five island or archipelago groups**, drawing on repeated interaction data from Hawaii (Aslan et al., 2019), Mallorca (Lázaro et al., 2022), Tenerife (Lara-Romero et al., 2019), Cabrera (Serra-Marin et al., 2025), Martinique (Cyrille, 2025) and Great Britain STEP (Holzschuh et al., 2016), with the latter harmonized in EuPPollNet (Lanuza et al., 2025). The source-balanced `D1` q90/q10 ratio was **4.521**, the `phi` q90–q10 span was **0.352**, the absolute source-balanced Spearman correlation between `log D1` and `phi` was **0.325**, and **26.2%** of systems occupied the joint interquartile interior.

Before extracting candidate systems we specified minimum numerical coverage criteria to guard against interpreting a narrow or effectively one-dimensional sample as broad regime occupancy. The observed plane exceeded each criterion. It also remained broad after removing all 19 systems from Mallorca, the largest study: `D1` q90/q10 was **5.494**, `phi` span remained **0.352**, interior occupancy was **26.1%** and `|rho_s|` was **0.454**.

The last admitted source, England STEP, expanded the high-synchrony edge. Its island-study classification came from EuPPollNet before our analysis. Before coordinates were opened, we froze four source-native rounds in each of 2012 and 2013, equal 30-minute and 300-m2 effort, and admitted only three sites with complete 4+4 outcome-independent survey schedules. Their `phi` values were 0.450, 0.635 and 0.414. An independent reconstruction reproduced every committed `D1` and `phi` exactly; removing zero-interaction rounds only for diagnosis retained high `phi` values of 0.411, 0.618 and 0.364.

An additional leave-one-study-out diagnostic reveals where replication remains thin. Removing England reduces the source-balanced `phi` span to **0.162**; removing Martinique lowers interior occupancy to **18.75%**. These diagnostics show that the present natural plane does not redundantly sample every region from multiple sources. We therefore interpret the plane as evidence that the two-dimensional regime space is real and broadly occupied, not as evidence that each part of that space is source invariant.

The result-blind source search was stopped once the prespecified coverage and largest-source sensitivity criteria were satisfied. Unresolved sources remained unopened rather than being pursued to broaden an already adequate plane.
...
Several boundaries remain. The natural systems do not validate the synthetic `C/I` crossover, and natural Hill `D1` is not synthetic `k`. The numerical natural-coverage criteria are analysis-design rules, not biological thresholds. The all-source diagnostic shows that the natural plane still has source-specific leverage. The consumer–resource generalization was motivated after the original plant–pollinator pattern, although the feedback intervention itself used unused seeds after prospective freezing. These boundaries distinguish a falsifiable claim about regime-dependent determinant order from a universal claim that the available evidence does not support.
...
For cross-system summaries, each source study received equal total weight. Before candidate extraction, we specified minimum coverage requirements for system/study/group counts, breadth dispersion, synchrony dispersion, two-dimensionality, joint-interior occupancy and persistence of the numerical dispersion criteria after removal of the largest study. These criteria were analysis-design safeguards against promoting a narrow or single-source natural sample to the main comparative claim; they are not ecological thresholds.
...
Admission records, source hashes, adapters, frozen analysis plans, route checkpoints, search closure and post-hoc transparency diagnostics are versioned in the repository. The England source was admitted and its adapter frozen before its coordinates were opened. A second implementation independently reconstructed its `D1` and `phi` values exactly. The all-source leave-one-study-out analysis was performed only after the primary coverage decision and is reported as a transparency diagnostic rather than a retroactive source-selection rule.

## Data availability

The natural-regime analysis uses public source datasets cited below. Source locks, checksums and access provenance are recorded in the repository. Public source data retain their original licences and repository records.

## Code availability

All analysis code, frozen design objects, failed and successful prediction receipts, natural-regime coordinates and provenance diagnostics are maintained in `izu-core`. A permanent archived release and DOI will replace the repository-only code statement before publication.
...
**Figure 3 | Natural breadth–synchrony regime plane.** Forty-two systems coded by source and island group with source-balanced marginal summaries. The inset shows removal of the largest source; supplementary source-deletion diagnostics identify the England high-synchrony and Martinique interior-coverage leverage.
