...
The present analysis asks four linked questions. First, **response geometry:** across plant starting positions, where does the island-minus-mainland functional-service contrast change sign? Second, **joint robustness:** does mixed-sign geometry survive across a broad declared parameter region, or only in a narrow selected corner? Third, **local context:** at what filtering strengths does realized local context change response sign rather than only magnitude? Fourth, **autonomous assurance:** how strongly must the implemented compensating route be amplified before a service-decline lineage crosses the reproductive sign boundary?
...
For each lineage contrast, we recorded whether the reproduction-response sign differed from its zero-filtering baseline and the first non-zero filtering strength at which that change occurred. The primary summaries were the number of lineage contrasts that changed sign somewhere in the envelope and the median first sign-change strength among those contrasts.
...
The result instead shows that local realization can reallocate branch identity. Depending on lineage and realized context, stronger filtering can either move a response toward a less negative/positive state or push it in the opposite direction. Local context is therefore a bidirectional branch allocator with buffering capacity, not a universal buffer.
...
The central result is that one broad pollinator reorganization does not map to one plant response. Its sign depends on where a lineage begins in functional matching space. This moves the inference away from a categorical claim that heterogeneity is necessary and toward a quantitative question about the geometry of the perturbation.

The approximately U-shaped baseline response shows why an average island effect can conceal mechanistically distinct outcomes. If lineages occupy different regions of functional space before pollinator reorganization, the same change in partner richness, turnover, generalization and replacement can improve matching for some lineages while reducing it for others. The important unit is therefore the position of a lineage relative to the functional reorganization, not island status alone.
...
This regime structure is more informative than a single branching count. It implies that bidirectional response is conditional on the combined geometry of partner traits, breadth, turnover, replacement and service saturation. A useful next theoretical step is to characterize the boundaries among these regimes analytically or with a denser targeted transition-surface design, rather than to search for more seeds that increase the number of mixed outcomes.
...
The broader conceptual framework distinguishes assembly filtering, in-situ evolution and post-establishment interaction response. The present simulation supports only the third layer directly.
...
First, the matching coordinate is abstract. The sign-switch intervals are model coordinates, not calibrated thresholds for a named floral trait. Second, the baseline mainland-like and island-like scenarios encode directional ecological assumptions but are not fitted estimates of one real archipelago. Third, 24 lineages, 120 steps and the saturation envelope are synthetic design choices. Fourth, the current trait-adjustment component is too weak to support conclusions about evolutionary dynamics. Fifth, the local-context threshold is a sensitivity property of the declared filtering implementation, not a field-estimated habitat threshold. Finally, no empirical system currently closes the full chain from matched local interaction context through visitor-specific rate and direct per-visit effectiveness to reproductive outcome.

# Conclusions

Post-establishment island response is not inherently monotonic. In the declared model, pollinator reorganization produces a response geometry in which starting functional position determines whether island-minus-mainland functional service increases or decreases. That geometry persists across part of a broad joint parameter design while collapsing to uniformly positive or negative responses elsewhere. Local availability / interaction filtering can change branch identity, whereas autonomous assurance attenuates response magnitude without generating sign rescue in the tested envelope.

The resulting picture is a conditional island response architecture: **common broad perturbation, position-dependent matching consequence, context-dependent branch realization, and downstream magnitude filtering**. This architecture is a model-side Research Article candidate. Its next gate is not additional seed search or mechanism tuning, but a claim/evidence audit of this regenerated manuscript and, separately, empirical admission of directly measured mechanisms where the broader comparative programme requires them.
