...
We next tested whether this failure occurs in ecological models with different internal structures. In a plant–pollinator response model, plant state interacts with stochastic partner identity, trait matching, breadth, arrival, loss and effectiveness. Across six frozen seeds, median normalized `(S,C,I)` changed from `(0.026, 0.730, 0.247)` at `k=1` to `(0.273, 0.235, 0.495)` at `k=4` and `(0.558, 0.127, 0.320)` at `k=16`. `I/C` rose from 0.34 to 2.52, so community and interaction contributions reversed order.
...
A stricter leave-one-study-out diagnostic shows where replication remains thin. Removing England reduces source-balanced `phi` span to **0.162**; removing Martinique lowers interior occupancy to **18.75%**. Thus the current plane demonstrates existence and broad occupancy of a two-dimensional regime space, not redundant sampling of every region by multiple sources. Source searching stopped once the prespecified coverage and largest-source sensitivity criteria were satisfied; unresolved sources remained unopened.
...
The natural regime plane supplies an external ecological constraint. Six studies occupy a broad breadth–synchrony space under a result-blind admission protocol, and the prespecified coverage criteria survive removal of the largest source. The later all-source sensitivity analysis is deliberately less reassuring: England supplies much of the sampled high-synchrony edge and Martinique materially supports interior occupancy. The strongest natural inference is therefore that the response-relevant context space is real and broadly occupied, not that every region is already independently replicated.
...
The plant–pollinator model evaluated 21 focal starting states across stochastic community realizations and pooled independent trajectories over `k={1,2,4,8,16}`. The adaptive consumer–resource model crossed resource number, matching width, handling, adaptation and aggregation levels over six frozen seeds. The failed handling-curvature prediction was frozen before its setting-level outcome map was opened.
...
**Figure 3 | Natural breadth–synchrony regime plane.** Forty-two systems coded by source and island group with source-balanced summaries. The largest-source sensitivity is prespecified; later source-deletion diagnostics identify England high-synchrony and Martinique interior-coverage leverage.
