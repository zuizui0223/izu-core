...
**Tier-C pipeline (implemented).** `paper/build_photo_scoring_sheet.py` pulls research-grade iNaturalist photos per region (mainland + each island) for a species and emits a **shuffled blinded sheet** ({card_id, image_url}) plus a **separate hidden key** (card_id→region), so relative floral traits are scored without seeing the island. A vision pass (or human) fills the sheet; join to the key only afterwards to get the mainland-vs-island direction. Verified end-to-end on *Hydrangea macrophylla* (17 cards; blinded score of a sample image succeeded). Caveat surfaced: the scored trait must be species-appropriate — for *Hydrangea* the display unit is sterile-floret size/number, not a corolla — so per-species trait definitions are required. Regenerable outputs live in `paper/photo_sheets/` (git-ignored).

**Tier-C scoring attempted to the limit (recorded honestly).** A real blinded-scoring batch (`paper/photo_scores.csv`) exposed a third, decisive limit beyond geographic sparseness: **phenology**. Of 4 sampled research-grade *Farfugium japonicum* photos, only 1 was in flower (the rest vegetative/fruiting); no in-flower *island* photo appeared in the sample, so **no mainland-vs-island direction was extractable for this generalist**. Combined with (a) per-island sparseness (Toshima ≈ 0), (b) no scale for absolute size, and (c) species-specific display units (Hydrangea = sterile florets), the conclusion is that single-source citizen photos can supply only coarse colour/presence traits for a few well-photographed species×islands — **not a systematic generalist-control size dataset**. The blinded pipeline and protocol are in place and correct; the ceiling is data, not method. Rigorous weight therefore stays with Tier-1 literature, and completing the generalist falsification test would require pooled multi-source photos with a phenology filter, or targeted fieldwork. This is the documented data ceiling, not a gap hidden by omission.
