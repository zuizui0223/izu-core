# `paper/` — current offline diagnostics and historical screening archive

This directory is **not** an authoritative manuscript surface. The current scientific claim boundary is defined by [`docs/CURRENT_EVIDENCE_STATE.md`](../docs/CURRENT_EVIDENCE_STATE.md), and executable work is routed by [`data/design/active_development_mainline.json`](../data/design/active_development_mainline.json).

## Active contents

The intentionally small current offline runner is:
...
1. the current evidence-state report;
2. the regime-transition registry validator; and
3. the retained focal anti-cline threshold diagnostic.

Those later diagnostics are design / claim-boundary aids. They do not promote a causal mechanism or create cross-lineage replication.

`paper/NOVELTY.md` is retained because it is part of the prior-art lock: the generic claim that reduced or smaller island pollinators can favour floral compression is prior art, not the novelty of this repository. The machine-readable companion is `data/design/pollinator_potential_prior_art.json`.
