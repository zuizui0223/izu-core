...
> **Historical archive — not an active development surface.**
>
> The U0 -> U1 -> Crossref broad-discovery chain described below was an exploratory search programme. As of 2026-08-14 it is retired from the active tree because it does not feed the current source-admission, source-native holdout, cross-archipelago effect registry, or Issue #91 direct effective-dependency pipeline. The historical screening tables remain as audit evidence; they must not be interpreted as current instructions to repeat exhausted or low-leverage occurrence/literature discovery.
>
> Current routing is defined by `data/design/active_development_mainline.json`, `docs/CURRENT_EVIDENCE_STATE.md`, `data/design/independent_primary_source_acquisition.json`, and the Issue #91 field pipeline. A new lawful source artifact may reopen a source-controlled gate, but that is distinct from rebuilding U0/U1.
...
**Historical procedure only; do not restart as routine development work.** The text below records how the retired expansion was framed.
...
**Historical search protocol only.** Current source-controlled reopen gates act on newly supplied or newly recovered admissible source material rather than rerunning this broad search surface.
